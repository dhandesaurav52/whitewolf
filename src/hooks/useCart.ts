...
export function useCart() {
  ...
  const { subtotal, comboDiscount } = useMemo(() => {
    let calculatedSubtotal = cart.reduce((total, item) => {
      // Safely parse the price, falling back to the current price if original is not available
      const basePrice = parseFloat(item.product.originalPrice || item.product.price);
    ...
    // Create a set of item unique IDs that are part of any combo deal.
    const itemsInComboDeals = new Set<string>();
    ...
    for (const ad of comboAds) {
      ...
      if (totalEligibleQuantity >= groupSize) {
        ...
        const potentialDiscountItems = itemsForAd
          .flatMap(item => Array(item.quantity).fill({product: item.product, size: item.size}))
          .sort((a, b) => parseFloat(a.product.price) - parseFloat(b.product.price));
        
        const itemsToDiscount = potentialDiscountItems.slice(0, numberOfTimesToApply * getQuantity);

        itemsToDiscount.forEach(discountedItem => {
          calculatedComboDiscount += parseFloat(discountedItem.product.price);
          const uniqueId = `${discountedItem.product.id}-${discountedItem.size || 'default'}`;
          itemsInComboDeals.add(uniqueId);
        });
      }
    }
    ...
    const itemsEligibleForStandardDiscount = cart.filter(item => {
      ...
      return !itemsInComboDeals.has(uniqueId);
    });
