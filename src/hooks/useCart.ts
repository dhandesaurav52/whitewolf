...
export function useCart() {
  ...
  const clearCart = useCallback(() => {
    ...
    let calculatedSubtotal = cart.reduce((total, item) => {
      const basePrice = item.product.originalPrice
    ...
    const comboAds = ads.filter(
      (ad) =>
        ad.discountType === 'buy-x-get-y' && ad.appliesTo === 'categories'
    );
    const allComboCategories = comboAds.flatMap((ad) =>
      ad.selectedCategories.map((c) => c.toLowerCase())
    );

    let itemsInComboDeals: string[] = [];

    // Process combo discounts first
    for (const ad of comboAds) {
      const categoriesForAd = ad.selectedCategories.map((c) =>
        c.toLowerCase()
      );
      const itemsForAd = cart.filter((item) =>
        categoriesForAd.includes(item.product.category.toLowerCase())
      );

      itemsForAd.forEach(item => {
        const uniqueId = `${item.product.id}-${item.size || 'default'}`;
        if (!itemsInComboDeals.includes(uniqueId)) {
          itemsInComboDeals.push(uniqueId);
        }
      });

      const totalEligibleQuantity = itemsForAd.reduce(
      ...
      if (totalEligibleQuantity >= groupSize) {
        const numberOfTimesToApply = Math.floor(
          totalEligibleQuantity / groupSize
        );
        const itemsToDiscount = itemsForAd
    ...
    const itemsEligibleForStandardDiscount = cart.filter(item => {
        const uniqueId = `${item.product.id}-${item.size || 'default'}`;
        return !itemsInComboDeals.includes(uniqueId);
    });

    // Process standard discounts on the remaining items
    const standardAds = ads.filter(
      (ad) =>
        (ad.discountType === 'percentage' || ad.discountType === 'fixed') &&
        ad.appliesTo === 'categories'
    );
    let standardDiscountTotal = 0;
    for (const item of itemsEligibleForStandardDiscount) {
      const applicableAd = standardAds.find((ad) =>
        ad.selectedCategories
          .map((c) => c.toLowerCase())
          .includes(item.product.category.toLowerCase())
      );
      const basePrice = item.product.originalPrice
        ? parseFloat(item.product.originalPrice)
        : parseFloat(item.product.price);
      ...
      if (applicableAd) {
        if (applicableAd.discountType === 'percentage') {
          standardDiscountTotal +=
            (basePrice * (applicableAd.discountValue / 100)) *
            item.quantity;
    ...
    return {
      ...
      comboDiscount: calculatedComboDiscount + standardDiscountTotal,
    };
  }, [cart, ads]);
